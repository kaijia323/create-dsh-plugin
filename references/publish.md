...
> Source: https://deepseek-harness.github.io/deepseek-harness/develop/basic/publish.html (raw: docs/user/develop/basic/publish.md)

The previous tutorials loaded a local plugin through a `--patch` overlay. This tutorial packages it as an installable **bundle**, installs it into a **profile** with `dsh plugin add`, and explains the layer order that determines the composed configuration. It assumes the `dsh` CLI is installed.
...
You never write a profile manifest by hand: `dsh plugin` creates and maintains it.
...
App arguments are not another patch layer. A surface bundle can resolve them through an ordinary app-owned service.
