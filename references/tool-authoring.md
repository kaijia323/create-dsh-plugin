...
> Source: https://deepseek-harness.github.io/deepseek-harness/cookbook/adding-a-tool.html (raw: docs/cookbook/adding-a-tool.md, deepseek-ai/deepseek-harness)
...
The neutral vocabulary lives in `dsh-tools`; tools never import a UI or transport type. Host/client runtimes map each `card` into their own view. `dsh-tool-fs` (generic/diff) and `dsh-tool-bash` (terminal) are the reference implementations.
